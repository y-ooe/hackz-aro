...
server.tool("get_trending_ideas", "topaz.devの最新プロジェクト・レシピからアイデアと使用技術を取得します。ユーザーが「何でもいいから作って」と言った時に使います。", {
    type: z
        .enum(["projects", "recipes", "both"])
        .default("both")
        .describe("取得するコンテンツ種別"),
}, async ({ type }) => {
    const targets = type === "both"
        ? ["projects", "recipes"]
        : [type];
    const allArticles = [];
...
// ツール2: 技術タグで絞り込み検索
server.tool("search_by_technology", "topaz.devで指定した技術タグのプロジェクト・レシピを検索します。ユーザーが特定の技術でコーディングを依頼した時に使います。", {
    tag: z
        .string()
...
// ツール3: 記事本文取得
server.tool("get_article_content", "topaz.devのプロジェクト・レシピURLを指定して本文を取得します。実装の詳細やコードを参照する時に使います。", {
